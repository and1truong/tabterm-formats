...
To update: re-copy `packages/module-host/{src,package.json,tsconfig.json}` from
tabterm into `vendor/module-host/` (or run `make vendor TABTERM=<path>`), bump the
version, and re-run `bun install`.
